...
public class ModFireSomersaultEffect extends AbstractGameEffect {
    private float x;
    private float y;
    ...
    public ModFireSomersaultEffect(float x, float y, int power) {
        this.duration = 0.5F;
        this.x = x;
        this.y = y;
        this.power = power;
    ...
    public void update() {
        ...
        for (int i = 1; i < power; i++) {
            float tmp = Interpolation.fade.apply(30.0F, -209.0F, this.duration / 0.5F) * ((float) Math.PI / 180F);
            this.v.x = MathUtils.cos(tmp) * X_RADIUS * (1F + (i * 0.2F) );
            this.v.y = -MathUtils.sin(tmp) * Y_RADIUS * (1F + (i * 0.2F) );
        ...
        // for (int i = 1; i <= power; i++) {
        //     this.v.x = MathUtils.cos(tmp * i) * X_RADIUS * i;
        //     this.v.y = -MathUtils.sin(tmp * i) * Y_RADIUS * i;
        //     AbstractDungeon.effectsQueue.add(new TorchParticleXLEffect(this.x + this.v.x + MathUtils.random(-30.0F, 30.0F) * Settings.scale, this.y + this.v.y + MathUtils.random(-10.0F, 10.0F) * Settings.scale));
        //     AbstractDungeon.effectsQueue.add(new TorchParticleXLEffect(this.x + this.v.x + MathUtils.random(-30.0F, 30.0F) * Settings.scale, this.y + this.v.y + MathUtils.random(-10.0F, 10.0F) * Settings.scale));
        //     AbstractDungeon.effectsQueue.add(new TorchParticleXLEffect(this.x + this.v.x + MathUtils.random(-30.0F, 30.0F) * Settings.scale, this.y + this.v.y + MathUtils.random(-10.0F, 10.0F) * Settings.scale));
        //     AbstractDungeon.effectsQueue.add(new ExhaustEmberEffect(this.x + this.v.x, this.y + this.v.y));
